class InstagramExternalConnector < Incollage::ExternalAccountConnector::Base

  def connect(controller, user_id)
    url = redirect_url(controller, user_id)
    controller.redirect_to InstagramConnector.new(url).authorize_url
  end

  def callback(controller, user_id)
    url = redirect_url(controller, user_id)
    response = InstagramConnector.new(url).get_response(controller.params[:code])
